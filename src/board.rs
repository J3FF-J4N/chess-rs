...
impl Board {
            ...
            self.grid.iter_mut().enumerate().for_each(|(row_idx, row)|{
                row.iter_mut().enumerate().for_each(|(col_idx, piece)| {
                    if let Some(piece_ref) = piece {
                        if piece_to_update.get_colour() == piece_ref.get_colour() && piece_to_update.get_type() == piece_ref.get_type() && piece_to_update.piece_id == piece_ref.piece_id {//Remove the old location
                            *piece = None;
                        }
                    }
                    if piece_to_update.pos_x == col_idx as u8 && piece_to_update.pos_y == row_idx as u8 { //Add to new location
    ...
    pub fn get_valid_moves(&mut self, piece: &Piece) -> Vec<(u8, u8)>{
        ...
        let moves = piece.get_valid_moves(); 
